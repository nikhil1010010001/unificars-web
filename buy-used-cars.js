...
import BuyCarSearchFilter from "@/components/BuyCarSearchFilter";
...
const buyUsedCars = () => {
  ...
  const fetchCarList = async () => {
    ...
    const fecthCars = await fetch(
      "https://crm.unificars.com/api/getcustomercars"
    );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 my-2 m-auto  rounded-xl py-4 pr-8">
            {CarListing && CarListing.length > 0 ? (
              CarListing.map((car, index) => {
                return (
                  <div
                    className=" transform px-4 pt-4 border rounded shadow-[0_0px_4px_1px_rgba(80,74,69,0.1)] transition-transform duration-300 hover:scale-105"
                    key={index}>
                    <div>
                      <Carousel
                        showStatus={false}
                    ...
                    <div className="cursor-pointer">
                      <div className="flex flex-col font-medium">
                        <Link href={`/cars/${car.lead.id}`} className="my-2">
                          <h5 className="text-sm my-1 hover:text-blue-900 ">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {
                                <div
                                  className={`uppercase font-normal items-center text-xs flex-nowrap text-white ${
                                    index % 2 == 0
                                      ? "bg-green-700"
                                      : "bg-orange-500"
                                  } p-1 rounded-full shadow-xl tracking-widest text-[8px]`}>
                                  <VerifiedUserIcon className="text-sm mr-1" />
                                  {index % 2 == 0 ? "certified" : "assured"}
                                </div>
                              }
                            </div>
                          </h5>
                          <div className="flex justify-between">
                            <div>
                              <span className="whitespace-nowrap text-base font-bold">
                                ₹{" "}
                                {parseInt(
                                  car.lead.selling_amount
                                ).toLocaleString("en-IN")}
                              </span>
                              <span className="font-semibold ml-2 line-through text-orange-500">
                                {" "}
                                ₹
                                {(
                                  parseInt(car.lead.selling_amount) +
                                  parseInt(car.lead.selling_amount) / 10
                                ).toLocaleString("en-IN")}{" "}
                              </span>
                            </div>
                          </div>
