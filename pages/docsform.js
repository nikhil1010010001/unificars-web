...
const docsform = () => {
    ...
    try {
        const response = await fetch('https://crm.unificars.com/api/customerdocumensts', {
        method: 'POST',
        body: formData,
        });
    ...
    return (
        <div className=''>
            <div className='setmaindiv bg-[url("/background/abtbg.jpg")]'  >
                ...
                {/* {showForm ? ( */}
                    <div className='setmaindiv2  left-[60%] top-5 right-[5%] mx-auto p-7 rounded' style={{ backgroundColor: "#e3810242" }}>
                    <h3 className="font-bold text-2xl my-2 capitalize">Upload Your <span className='text-orange-500'>Necessary Document For the Further Process</span></h3>
                    <div className=""><img src={'/title_line.png'} alt="" /></div>
                    <form onSubmit={HandleFormSubmit} className="my-5 text-lg font-bold flex flex-col gap-4 text-gray-500" id="enquiry">
                        <div className="form-grp">
                        <label className="flex gap-2 my-2" htmlFor="adharcard">Aadhar Card <span className='text-orange-500'>*</span></label>
                        <input required type="file" onChange={UserInputChange} className='p-3 w-full focus:outline-none rounded-xl shadow' name="adharcard" id="adharcard" accept="image/*" />
                        </div>
                        <div className="form-grp">
                        <label className="flex gap-2 my-2" htmlFor="pancard">Pan Card <span className='text-orange-500'>*</span></label>
                        <input required type="file" onChange={UserInputChange} className='p-3 w-full focus:outline-none rounded-xl shadow' name="pancard" id="pancard" accept="image/*" />
                        </div>
                        <div className="form-grp">
                        <label className="flex gap-2 my-2" htmlFor="cancelCheque">Cancel Cheque <span className='text-orange-500'>*</span></label>
                        <input required type="file" onChange={UserInputChange} className='p-3 w-full focus:outline-none rounded-xl shadow' name="cancelCheque" id="cancelCheque" accept="image/*" />
                        </div>
                        ...
                        <button type='submit' disabled={loading} className={`bg-orange-400 rounded-xl shadow p-2 border disabled:border-0 border-orange-500 text-white ${loading ? 'disabled:bg-orange-300' : 'disabled:bg-orange-300'}`}>
