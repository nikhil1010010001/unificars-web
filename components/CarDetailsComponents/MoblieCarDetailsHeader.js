...
const MoblieCarDetailsHeader = (props) => {
    ...
    return (<div className='grid grid-cols-1 justify-around'>
        {car ?
            <><div className='grid grid-cols-2'>
                <div>
                    <h1 className="text-black/80 text-base font-bold mb-1">{car && car.lead ? car.lead[0]["Brand"] : ""}  </h1>
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <div>
                        <button className="flex text-[#f38102] border-[1px] border-[#f38102] text-xs active:text-white active:bg-[#f38102]  p-2 focus:outline-none items-center rounded-xl" onClick={() => setShowEnquiryModal(true)}>
                            <p>ENQUIRE</p> <AiOutlineArrowRight className="mx-1" /></button>
                    </div>
                </div>
            </> : <></>}
    </div>)
