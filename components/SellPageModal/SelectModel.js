...
const SelectModel = ({ setCarInfo, carInfo, setValue, value }) => {
    ...
    return (
        <>
            {!Loading ? <div className=' mx-auto  gap-4  lg:grid-cols-3 grid grid-cols-2'>
                {BrandsModels.length > 0 ? BrandsModels.map((e, index) => {
                    return <div  onClick={() => handleClick(e)} className='col-span-1 shadow-md border p-2 cursor-pointer hover:shadow-xl hover:scale-105 transition-all duration-200 hover:bg-[#f38102] hover:text-white rounded' key={index}>
