...
const SelectVariant = ({ setCarInfo, carInfo, value, setValue }) => {
  ...
  const handleClick = (varient) => {
    setCarInfo({
      ...carInfo,
      variant: { id: varient.id, name: varient.varient },
    });
            ...
            BrandsModelVareints.map((e, index) => {
              return (
                <div
                  ...
                  className="shadow-md py-2 px-4 cursor-pointer hover:shadow-xl hover:scale-105 transition-all duration-200 hover:bg-[#f38102] hover:text-white rounded-lg"
                  key={index}>
