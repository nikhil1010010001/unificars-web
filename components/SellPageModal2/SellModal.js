...
const SellModal = ({setVerifyNum, OpenModal, setOpenModal, UserCar, setUserCar ,UserCarTabArray ,setUserCarTabArray ,CurrentTab ,setCurrentTab }) => {
    ...
    return (
        <>
            <div className={`fixed bg-black/40 top-0 w-full ${OpenModal ? "h-full" : "h-0"} transition-all duration-200 flex items-center justify-center`}>
                <div className='bg-white w-5/6 h-5/6 mx-auto overflow-y-auto rounded'>
                    <div className='overflow-x-scroll sticky bg-white top-0  p-4 z-50'>
                        <ul className='flex gap-4 text-xl  mb-1 '>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='brand' onClick={(e)=>HandleDetailBarClick(e,"0")}>{(UserCar.brand).name==""?"Brand":(UserCar.brand).name}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='model' onClick={(e)=>HandleDetailBarClick(e,"1")}>{(UserCar.model).name==""?"Model":(UserCar.model).name}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='varient' onClick={(e)=>HandleDetailBarClick(e,"2")}>{(UserCar.varient).name==""?"Varient":(UserCar.varient).name}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='year' onClick={(e)=>HandleDetailBarClick(e,"3")}>{UserCar.year==""?"Year":UserCar.year}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='ownerShip' onClick={(e)=>HandleDetailBarClick(e,"4")}>{UserCar.ownerShip==""?"Ownership":`ownership : ${UserCar.ownerShip}`}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='fuelType' onClick={(e)=>HandleDetailBarClick(e,"5")}>{UserCar.fuelType==""?"Fuel Type":UserCar.fuelType}</li>
                            <li className='bg-gray-200 whitespace-nowrap font-normal text-gray-400 p-2 px-4 cursor-pointer rounded shadow' id='kmDriven' onClick={(e)=>HandleDetailBarClick(e,"6")}>{UserCar.kmDriven==""?"Km Driven":UserCar.kmDriven}</li>
                                ...
                                case "4":
                                    ...
                                    case "7":
                                        return (
                                            ...
                                            <SelectStates setOpenModal={setOpenModal} UserCarTabArray={UserCarTabArray}  setUserCar={setUserCar} UserCar={UserCar} setCurrentTab={setCurrentTab} setVerifyNum={setVerifyNum} />
                                        </>
                                    )
                            }
                        })()}
