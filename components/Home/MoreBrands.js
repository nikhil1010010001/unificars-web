...
const MoreBrands = ({setOpenModal ,setUserCar, UserCar, UserCarTabArray ,setCurrentTab, OpenModal }) => {
    ...
    return (
        <>
            {!Loading ? (
                <div className={`grid grid-cols-3 md:grid-cols-6 lg:grid-cols-4 ${OpenModal ? "w-11/12" : "w-11/12"} gap-4 my-8`} >
                    {(showAll ? BrandsLogo : BrandsLogo.slice(0, 10)).map((e, index) => (
                        <div 
                            onClick={() => {
                                setUserCar({ ...UserCar, brand: { name: e.brand_name, id: e.id } }); 
                            ...
                            key={index}>
                            <Image priority width={60} height={80} src={e.image} alt={e.brand_name} className='' />
                        </div>
                    ))}
                    {!showAll && BrandsLogo.length > 10 && (
                        <button onClick={() => setShowAll(true)} className='shadow-md border w-full p-4 cursor-pointer hover:shadow-xl hover:scale-105 transition-all duration-200 hover:bg-gray-50 flex items-center justify-center rounded-2xl font-inter text-[#da251c] text-lg'>
                            More
                        </button>
                    )}
                </div>
